...
class NMRParametersCalculator:
    ...
    def calculate(self, 
        ...
        if self.config.calculate_T1:
            if self.config.interaction_type == 'CSA':
                T1 = self._calculate_T1_CSA(J_values)
    ...
    def _calculate_T1_CSA(self, J_values: Dict[str, float]) -> float:
        """
        Calculate T1 for CSA relaxation.
        
        R1 = (1/T1) = (2/15) × (ωN × Δσ)² × [J(ωN) + 3J(ωN) + 6J(2ωN)]
        
        Simplified (assuming J(2ωN) ≈ 0 for slow motion):
        R1 ≈ (8/15) × (ωN × Δσ)² × J(ωN)
        ...
        # Approximate J(2ωN) ≈ 0 for slow tumbling
        J_omega_N = J_values['J_omega_N']
        
        R1_csa = (2.0/15.0) * (delta_sigma_rad)**2 * 4 * J_omega_N
