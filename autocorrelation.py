...
Calculate autocorrelation functions for Y₂ₘ time series using efficient FFT method.
...
class AutocorrelationCalculator:
    ...
    def calculate(self, Y2m_coefficients: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for m_idx, m in enumerate([-2, -1, 0, 1, 2]):
            ...
            # Add to total (properly accounting for conjugate)
            if m < 0:
                # For m < 0, need conjugate
    ...
    def _calculate_acf_fft(self, 
        ...
        """
        Calculate autocorrelation function using FFT method.
        
        C(τ) = ⟨x(t) × x*(t+τ)⟩
        
        FFT method:
        1. Compute power spectrum: P(ω) = |FFT(x)|²
        2. Inverse FFT: C(τ) = IFFT(P(ω))
        ...
        # Zero-pad to 2n for proper FFT
        series_padded = np.concatenate([series, np.zeros(n)])
    ...
    @staticmethod
    def calculate_acf_direct(series: np.ndarray, max_lag: int) -> np.ndarray:
        """
        Calculate ACF using direct method (slower but clearer).
        
        Useful for validation of FFT method.
        ...
        for lag in range(max_lag):
            if lag == 0:
                acf[lag] = np.mean(np.abs(series)**2)
            else:
                overlap = series[:-lag] * np.conj(series[lag:])
                acf[lag] = np.mean(overlap).real
